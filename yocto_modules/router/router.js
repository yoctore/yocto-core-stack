...
router.prototype.check = function(app, route) {
   ...
   var rules = new Array({ 'property' : 'method', 'required' : true, 'rules' : 'get|post|delete|put'},
                         ...
                         { 'property' : 'controller', 'required' : true, 'properties' : ['name', 'func', 'extra']}
                        );
   /**
                ...
                if (p === 'controller') {
                  ...
                  if (current === 'func') {
                    if (_.isUndefined(controller) || _.isUndefined(controllerName)) {
                      logger.warning('Invalid usage for controller config. your properties must be in this order : [name, func, extra]');
                      return false;
                    }
